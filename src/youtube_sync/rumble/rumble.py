...
def fetch_rumble_channel_today_legacy(
    ...
    for article in soup.find_all("div", class_="videostream thumbnail__grid--item"):
        try:
            ...
            assert article_duration_dom is not None
...
def fetch_rumble_channel_today_partial_result(
    channel_name: str, channel: str
) -> list[PartialVideo]:
    ...
    for article in soup.find_all("div", class_="videostream thumbnail__grid--item"):
        try:
            ...
            vid_src_suffix = article.find("a", class_="videostream__link link")["href"]  # type: ignore
            vid_src = "https://rumble.com%s" % vid_src_suffix
            dom_vid_status = article.find("div", class_="videostream__status")  # type: ignore
            if dom_vid_status is not None:
                fuzzy_date = dom_vid_status.get_text().strip()
            else:
                fuzzy_date = ""
            title = parse_title(article)  # type: ignore
...
def parse_date(article_soup: BeautifulSoup) -> str:
    ...
    if dom_date is not None:
        ...
        if hasattr(dom_date, "title"):  # type: ignore
            date = dom_date["title"]  # type: ignore
            if date:
                return str(date)
    ...
    while True:
        ...
        try:
            ...
            for article in soup.find_all(
                "div", class_="videostream thumbnail__grid--item"
            ):
                try:

                    assert article is not None
                    duration = parse_duration(article)  # type: ignore
                    assert duration is not None

                    vid_src_suffix = article.find("a", class_="videostream__link link")[  # type: ignore
                        "href"
                    ]
                    assert vid_src_suffix is not None
                    vid_src = f"https://rumble.com{vid_src_suffix}"
                    fuzzy_date = parse_date(article)  # type: ignore
                    date = parse_fuzzy_date(fuzzy_date)  # type: ignore
                    title = parse_title(article)  # type: ignore
